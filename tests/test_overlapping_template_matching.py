...
def test_overlapping_all_zeros_fails():
    # All zeros should contain many overlapping matches of "00000000"
    N = 512
    bits = [0] * N
    b = bits_to_bytes(bits)
    view = BytesView(b)
    plugin = OverlappingTemplateMatching()
    params = {"template": "00000000", "min_bits": 256, "alpha": 0.01}

    res = plugin.run(view, params)
    assert isinstance(res, TestResult)
    assert res.test_name == "overlapping_template_matching"
    assert 0.0 <= res.p_value <= 1.0
    # Expect test to fail (very unlikely under randomness)
    assert res.passed is False
    assert res.metrics["observed_count"] >= 1
    assert res.metrics["total_bits"] == N


def test_overlapping_alternating_passes():
    # Alternating pattern '01' has very low chance of 8 zeros in a row
    N = 512
...
def test_overlapping_skipped_for_small_input():
