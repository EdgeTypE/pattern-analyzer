"""Command line interface for Pattern Analyzer."""
...
@click.option('--log-level', 'log_level', type=click.Choice(['DEBUG','INFO','WARNING','ERROR'], case_sensitive=True), default='INFO',
              help='Set logging level for patternanalyzer logger and test debug output')
...
def bench(calibrate, samples, seed, out_dir, profile):
    ...
    Usage:
      python -m patternanalyzer.cli bench --calibrate [--samples N] [--seed S] [--out-dir DIR] [--profile PROFILE]
    """
    try:
        if not calibrate:
...
def serve_ui(host, port, reload):
    ...
    """
    try:
        click.echo(f"Starting Pattern Analyzer Streamlit UI at http://{host}:{port}/")
        try:
            import shutil
            import subprocess
...
@cli.command()
def tui():
    """Start the Pattern Analyzer Textual TUI."""
    try:
        click.echo("Starting Pattern Analyzer Textual TUI...")
        from .tui import PatternAnalyzerTUI
