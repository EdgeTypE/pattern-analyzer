...
def _run_test_worker(module_name: str, class_name: str, test_name: str, data_bytes: bytes, params: dict):
    ...
    try:
        ...
        from patternlab.plugin_api import BytesView, TestResult  # type: ignore
...
def _run_test_subprocess(self, module_name: str, class_name: str, test_name: str, data_bytes: bytes, params: dict, timeout: float, mem_mb: int = None):
    """Run the plugin in an isolated subprocess by invoking patternlab.sandbox_runner.
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "patternlab.sandbox_runner"]
...
class Engine:
    ...
    def register_transform(self, name: str, plugin: TransformPlugin):
        ...
        try:
            plugin.logger = logging.getLogger(f"patternlab.plugins.{name}")
        except Exception:
            pass
    ...
    def _analyze_impl(self, input_bytes: bytes, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not parallel:
            ...
            for c in tests_conf:
                ...
                if missing_reqs:
                    ...
                    try:
                        logging.getLogger("patternlab.engine").debug("starting_test", extra={"test_name": c.get("name"), "params": params, "timeout_sec": timeout_sec, "bytes_processed": bytes_processed})
                    except Exception:
                        pass
                    try:
                        ...
                        try:
                            logging.getLogger("patternlab.engine").warning("test_timeout", extra={"test_name": c.get("name"), "timeout_sec": timeout_sec})
                        except Exception:
                            pass
                    except Exception as e:
                        ...
                        duration_ms = (end - start) * 1000.0
                        try:
                            logging.getLogger("patternlab.engine").exception("test_exception", extra={"test_name": c.get("name"), "err": str(e)})
                        except Exception:
                            pass
                    # Completed run: rich debug
                    try:
                        logging.getLogger("patternlab.engine").debug("finished_test", extra={"test_name": c.get("name"), "status": res.get("status") if isinstance(res, dict) else getattr(res, "passed", None), "time_ms": duration_ms})
                    except Exception:
                        pass
                ...
                for c, future in submissions:
                    ...
                    if future is None:
                        ...
                        try:
                            ...
                            try:
                                logging.getLogger("patternlab.engine").debug("starting_local_fallback", extra={"test_name": c.get("name"), "params": params, "timeout_sec": timeout_sec})
                            except Exception:
                                pass
                            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as tpool:
                                fut = tpool.submit(tp.safe_run if hasattr(tp, "safe_run") else tp.run, data, params)
                            ...
                            duration_ms = timeout_sec * 1000.0
                            try:
                                logging.getLogger("patternlab.engine").warning("local_fallback_timeout", extra={"test_name": c.get("name"), "timeout_sec": timeout_sec})
                            except Exception:
                                pass
                        except Exception as e:
                            res = {"test_name": c['name'], "status": "error", "reason": str(e)}
                            ...
                            duration_ms = (end - start) * 1000.0
                            try:
                                logging.getLogger("patternlab.engine").exception("local_fallback_exception", extra={"test_name": c.get("name"), "err": str(e)})
                            except Exception:
                                pass
                        if isinstance(res, TestResult):
                            try:
                        ...
                        try:
                            ...
                            try:
                                logging.getLogger("patternlab.engine").debug("future_completed", extra={"test_name": c.get("name")})
                            except Exception:
                                pass
                            if isinstance(res, TestResult):
                                raw_results.append(res)
                            ...
                            try:
                                ...
                                pass
                            try:
                                logging.getLogger("patternlab.engine").warning("future_timeout_cancelled", extra={"test_name": c.get("name"), "timeout_sec": timeout_sec})
                            except Exception:
                                pass
                            raw_results.append({"test_name": c['name'], "status": "error", "reason": "timeout"})
                        except Exception as e:
                            try:
                                logging.getLogger("patternlab.engine").exception("future_result_exception", extra={"test_name": c.get("name"), "err": str(e)})
                            except Exception:
                                pass
        ...
        try:
            meta["engine_version"] = importlib.metadata.version("patternlab")
        except Exception:
            ...
            try:
                ...
                if jinja2:
                    ...
                    try:
                        ...
                        # Ensure tests expecting a literal "<h1>PatternLab Report" string find it.
                        if "<h1>PatternLab Report" not in rendered:
                            idx = rendered.find('<div class="container">')
                            if idx != -1:
                                insert_at = idx + len('<div class="container">')
                                rendered = rendered[:insert_at] + "\n  <h1>PatternLab Report</h1>\n" + rendered[insert_at:]
